...
function shuffleAnswers(question) {
    ...
    // Rebuild answers with new A/B/C/D
    const newAnswers = {};
...
// Correct ladder order (bottom → top)
const moneyLevels = [
    "$100",
    "$200",
    "$300",
...
// Map question index to ladder level
function getLevelIndexFromQuestion(qIndex) {
    return Math.min(moneyLevels.length - 1, Math.floor(qIndex / 2));
}
...
function initLadder() {
    ...
    for (let i = 0; i < moneyLevels.length; i++) {
        ...
        const levelNumber = i + 1; // 1 at top, 15 at bottom
...
function updateLadder() {
    ...
    ladderItems.forEach(li => li.classList.remove("current", "past"));
    ...
    ladderItems.forEach(li => {
        ...
        if (idx === levelIndex) li.classList.add("current");
        else li.classList.add("past");
    });
}
...
function loadQuestion() {
    ...
    const q = shuffleAnswers(questions[currentIndex]);
...
function useAudience() {
    ...
    others.forEach((l, idx) => {
        if (idx === others.length - 1) poll[l] = remaining;
        else {
    ...
    document.getElementById("lifeline-result").textContent =
        `Audience poll → A: ${poll.A || 0}%  B: ${poll.B || 0}%  C: ${poll.C || 0}%  D: ${poll.D || 0}%`;
}
...
function useFriend() {
    ...
    if (friendIsRight) suggestion = correct;
    else {
    ...
    document.getElementById("lifeline-result").textContent =
        `Your friend says: "I think the answer is ${suggestion}."`;
}
